...
class VerbList(Resource):
    def __init__(self):
        ...
        self.parser.add_argument(
            "thematic-relation",
            dest="thematic_relation",
            type=str,
            location="args",
            required=False,
            help="A thematic relation to filter",
        )
        self.parser.add_argument(
            "tag",
            dest="tag",
    ...
    def addRequired(self, verb):
        if "required_affixes" in list(verb.keys()) and verb["required_affixes"]:
            if (
                type(verb["required_affixes"][0]) is str
                or type(verb["required_affixes"][0]) is str
            ):
    ...
    def get(self):
        ...
        if args["tag"]:
            return [
                self.addRequired(verb)
                for verb in verb_data
                if verb["tag"] in args["tag"]
            ], {"X-total-count": len(verb_data)}
        if thematic_relation:
            return [
                self.addRequired(verb)
                for verb in [
                    x for x in verb_data if thematic_relation == x["thematic_relation"]
                ]
            ], {"X-total-count": len(verb_data)}
        else:
            return [self.addRequired(verb) for verb in verb_data], {
                "X-total-count": len(verb_data)
            }
    ...
    def addRequired(self, verb):
        if "required_affixes" in list(verb.keys()) and verb["required_affixes"]:
            if (
                ...
                # return any tags in verb["required"] that match affix tags in affix_data
                verb["required_affixes"] = [
                    [x for x in affix_data if x["tag"] == morpheme]
                    for morpheme in verb["required_affixes"]
                ]
                return verb
            else:
...
api.add_resource(VerbList, "/verbs", endpoint="verbs")

api.add_resource(Verb, "/verbs/<string:tag>", endpoint="verb")
