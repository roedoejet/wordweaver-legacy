...
class PnOptions:
    ...
    def intransAgentPros(self, neutral_tag="3-sg-n"):
        return [
            {
                "agent": pronoun,
                "patient": [x for x in pronoun_data if x["tag"] == neutral_tag][0],
            }
    ...
    def intransPatientPros(self, neutral_tag="3-sg-n"):
        return [
            {
                "patient": pronoun,
                "agent": [x for x in pronoun_data if x["tag"] == neutral_tag][0],
            }
            for pronoun in pronoun_data
        ]
...
class PronounList(Resource):
    ...
    def get(self):
        ...
        if args["tag"]:
            return [
                pronoun for pronoun in pronoun_data if pronoun["tag"] in args["tag"]
            ], {"X-total-count": len(pronoun_data)}
        return [pronoun for pronoun in pronoun_data], {
            "X-total-count": len(pronoun_data)
        }
...
class Pronoun(Resource):
    ...
    def get(self, tag):
        try:
            selected_pronoun = next(
                pronoun for pronoun in pronoun_data if pronoun["tag"] == slugify(tag)
            )
...
api.add_resource(PronounList, "/pronouns", endpoint="pronouns")

api.add_resource(Pronoun, "/pronouns/<string:tag>", endpoint="pronoun")
